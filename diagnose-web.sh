...
$COMPOSE_CMD exec -T web sh -c 'echo "DATABASE_URL=$DATABASE_URL"' 2>/dev/null || echo "❌ Не удалось получить DATABASE_URL"
echo ""
...
echo "   В контейнере:"
if $COMPOSE_CMD exec -T web test -f /app/database/db.sqlite 2>/dev/null; then
    container_size=$($COMPOSE_CMD exec -T web stat -c%s /app/database/db.sqlite 2>/dev/null || echo "0")
    echo "   ✅ /app/database/db.sqlite существует (размер: ${container_size} байт)"
    $COMPOSE_CMD exec -T web ls -lh /app/database/db.sqlite 2>/dev/null || true
else
...
echo "   В контейнере:"
$COMPOSE_CMD exec -T web ls -ld /app/database 2>/dev/null || echo "   ❌ Директория /app/database не найдена"
$COMPOSE_CMD exec -T web ls -l /app/database/db.sqlite 2>/dev/null || echo "   ❌ Файл db.sqlite не найден"
echo ""

# 6. Проверка подключения к БД через Prisma
echo "6️⃣ Проверка подключения к БД через Prisma:"
$COMPOSE_CMD exec -T -w /app web sh -c '
...
$COMPOSE_CMD exec -T web sh -c 'env | grep -E "DATABASE_URL|ADMIN_|NEXTAUTH_" | sort' 2>/dev/null || echo "❌ Не удалось получить переменные окружения"
echo ""

# 8. Проверка .env файла на хосте и сравнение с контейнером
echo "8️⃣ DATABASE_URL в .env файле и сравнение:"
if [ -f ".env" ]; then
    env_db_url=$(grep "^DATABASE_URL=" .env | head -1 | sed 's/^DATABASE_URL=//' | tr -d '"' || echo "")
...
# Сравниваем с DATABASE_URL в контейнеre
if [ -n "$env_db_url" ]; then
    container_db_url=$($COMPOSE_CMD exec -T web sh -c 'echo "$DATABASE_URL"' 2>/dev/null || echo "")
    if [ -n "$container_db_url" ]; then
        ...
        if [ "$env_db_url" != "$container_db_url" ]; then
            ...
            echo "   Рекомендуется перезапустить контейнер: $COMPOSE_CMD restart web"
        else
...
$COMPOSE_CMD exec -T web netstat -tlnp 2>/dev/null | grep 3000 || \
$COMPOSE_CMD exec -T web ss -tlnp 2>/dev/null | grep 3000 || \
echo "   ⚠️  Порт 3000 не найден (возможно, приложение не запущено)"
echo ""

# 10. Проверка healthcheck
echo "🔟 Проверка healthcheck:"
$COMPOSE_CMD exec -T web node -e "
...
" 2>&1 || echo "   ❌ Healthcheck не прошел"
echo ""
